...
if (!actor || !base || !head) {
  console.error(
    "Ownership validation requires GITHUB_ACTOR, --base, and --head.",
  );
  process.exit(1);
}
...
if (instructorUsers.includes(actor)) {
  console.log(
    `Ownership validation skipped for instructor account "${actor}".`,
  );
  process.exit(0);
}
...
if (disallowedFiles.length > 0) {
  console.error(
    `Ownership validation failed for "${actor}" with assigned slug "${actorSlug}".`,
  );

...
const errors = [];
const studentFiles = (await fs.readdir(studentsDir)).filter((file) =>
  file.endsWith(".json"),
);
...
for (const studentFile of studentFiles) {
  ...
  // Should I lowercase this slug?
  const githubUsername = data.githubUsername;

  if (typeof slug !== "string" || !slugPattern.test(slug)) {
    errors.push(
      `${fullPath}: slug must use lowercase letters, numbers, and hyphens only.`,
    );
    continue;
  }
  ...
  if (slug !== expectedSlug) {
    errors.push(
      `${fullPath}: slug "${slug}" must match filename "${expectedSlug}".`,
    );
  }
...
for (const fullPath of markdownFiles) {
  ...
  if (!studentSlug || !fileName || rest.length > 0) {
    errors.push(
      `${fullPath}: posts must be stored as src/content/posts/{studentSlug}/YYYY-MM-DD-post-slug.md.`,
    );
    continue;
  }
  ...
  if (!studentSlugs.has(studentSlug)) {
    errors.push(
      `${fullPath}: no student profile exists for slug "${studentSlug}".`,
    );
  }
  ...
  if (postSlugsByStudent.has(duplicateKey)) {
    errors.push(
      `${fullPath}: duplicate post slug "${postSlug}" for student "${studentSlug}".`,
    );
  }
